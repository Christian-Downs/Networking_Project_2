...
void handle_client(int pid, struct sockaddr_storage their_addr, map<string, string> configMap)
{
  ...
  else if (configMap.contains("PEER_SUBNET") &&
           !configMap.at("PEER_SUBNET").empty() &&
           message_string.find("peer") != string::npos)
  {

    peer_handler(pid, s, message_string, configMap["PEER_SUBNET"]);
    close(pid);
    return;
  }
  else
...
int main(int argumentCount, char *argumentArray[])
{
  ...
  while (true)
  {
    ...
    std::jthread(handle_client, new_fd, their_addr, configMap).detach();
  }
